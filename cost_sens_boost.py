...
def IRLS(typecost, X, initial_residuals, a, b, **kwargs):
    ...
    if scale == True:
        X_save = X.copy()
        X, X_std_scores = rescale(X,X.columns)
    ...
    for i in range(0, n_iter):
        ...
        if i == 0:
            ...
            s_residuals = residuals.std()
            s_residuals = 1
        else:
            pred_list = X @ Beta
            pred = np.array(pred_list)
            pred.shape = [n, 1]
            residuals = initial_residuals - pred

        residuals_rescaled = residuals / 1

        # Apply derivative cost function to the residuals to get the weights
        if typecost == 'linlin':
            residuals_rescaled[residuals_rescaled == 0] = 0.00000001
            weights = np.where(residuals_rescaled >= 0, a / (np.abs(residuals_rescaled)),
                               b / (np.abs(residuals_rescaled)))
        if typecost == 'quadquad':
        ...
        pred = np.array(pred_list)
        pred.shape = [n, 1]
        residuals_convergence = initial_residuals - pred
        costs_convergence = cost_function(residuals_convergence, typecost, a, b)

        if (previous_convergence_costs - costs_convergence) > (s_residuals * 0.000000001):
    ...
    final_residuals_post_hoc_model = initial_residuals - predictions
